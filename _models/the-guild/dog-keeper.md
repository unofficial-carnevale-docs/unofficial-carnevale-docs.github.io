---
layout: model
title: Dog Keeper
permalink: /factions/the-guild/dog-keeper/
faction: the-guild
faction_title: The Guild
nav_section: Factions
cost: 12
meta: "Henchman · Size 30"
---

<div class="model-stats">

| AP | LP | WP | CP | Move | Dex | Atk | Prot | Mind |
|---|---|---|---|---|---|---|---|---|
| 2 | 11 | 1 | 2 | 4 | 4 | 3 | 3 | 2 |

</div>

### Weapons

| Name | Range | Damage | Pen | Special |
|---|---|---|---|---|
| Training Whip | 3" | — | — | — |

### Abilities

<div class="chips" markdown="0"><a class="chip" href="/compendium/character-abilities/#companion-x">Companion(Dog)</a><a class="chip" href="/compendium/character-abilities/#engage">Engage</a></div>

**"Encouragement"** - This character may only use the ORDER or COUNTER Commands on characters with the Henchman keyword.

## Capability

<div class="metric-tiles" markdown="0"><div class="tile"><span class="v">1.36</span><span class="k">EDPA</span></div><div class="tile"><span class="v">0.113</span><span class="k">EDPA/Duc</span></div><div class="tile"><span class="v">9.7</span><span class="k">ATK2K</span></div><div class="tile"><span class="v">0.81</span><span class="k">ATK2K/Duc</span></div></div>

<div class="metric-median" markdown="0"><p class="metric-median-cap">Support median</p><div class="metric-tiles is-median"><div class="tile"><span class="v">1.46</span><span class="k">EDPA</span></div><div class="tile"><span class="v">0.111</span><span class="k">EDPA/Duc</span></div><div class="tile"><span class="v">9.7</span><span class="k">ATK2K</span></div><div class="tile"><span class="v">0.75</span><span class="k">ATK2K/Duc</span></div></div></div>

[How these metrics are computed →](/mechanics/combat/)

<div class="model-pairs">
<div>

### EDPA by target

*Sustained: training whipx2*

| Target | EDPA | Median |
|---|---|---|
| Soft (Dex3/Prot1) | 2.48 | 2.41 |
| Balanced (Dex4/Prot3) | 1.36 | 1.46 |
| Evasive (Dex6/Prot2) | 1.23 | 1.30 |
| Armoured (Dex4/Prot5) | 0.82 | 0.87 |
| Tank (Dex4/Prot5/US5) | 0.82 | 0.67 |
| Parry (Dex5/Prot3/Parry2) | 1.13 | 0.92 |


</div>
<div>

### Defence

*Base: 11 LP*

| Attacker | Net LP/hit | ATK2K | Median |
|---|---|---|---|
| Baseline (Atk3) | 1.13 | 9.7 | 9.7 |
| Elite (Atk5, Dmg+1) | 3.38 | 3.3 | 3.3 |
| Penetrator (Atk3, Pen−3) | 2.20 | 5.0 | 5.3 |
| Drown (Atk3) | 1.90 | 5.8 | 5.8 |

</div>
</div>
