---
layout: model
title: Silere Priest
permalink: /factions/the-vatican/silere-priest/
faction: the-vatican
faction_title: The Vatican
nav_section: Factions
---

<p class="model-meta"><strong>Hero · Discipline(<a href="/magic/fateweaving/" class="disc-fateweaving">Fateweaving</a>, <a href="/magic/runes-of-sovereignty/" class="disc-runes-sovereignty">Runes of Sovereignty</a>)</strong><span>Size 30 · 16 Ducats</span></p>

<div class="model-stats">

| AP | LP | WP | CP | Move | Dex | Atk | Prot | Mind |
|---|---|---|---|---|---|---|---|---|
| 2 | 12 | 3 | 0 | 4 | 3 | 4 | 4 | 4 |

</div>

### Weapons

| Name | Range | Damage | Pen | Special |
|---|---|---|---|---|
| Fire of Persecution | melee | +1 | −1 | — |

### Abilities

[Mage(1)](/compendium/character-abilities/#mage-x), [Universal Shielding(2)](/compendium/character-abilities/#universal-shielding-x)

**Keeper of the Fire of the Persecution** — Any friendly characters without this ability within 3" gain +1 Damage on their weapons.

## Capability

EDPA **3.10** · EDPA/Duc **0.194** · ATK2K **11.2** · ATK2K/Duc **0.70**

*Support median: EDPA **1.46** · EDPA/Duc **0.111** · ATK2K **9.7** · ATK2K/Duc **0.75***

*Upper EDPA outlier: among this faction's Supports the median is 1.25, but this model reaches 3.10, so the role median understates models like this.*

[How these metrics are computed →](/mechanics/combat/)

<div class="model-pairs">
<div>

### EDPA by target

*Sustained: fire of persecutionx2*

| Target | EDPA | Median |
|---|---|---|
| Soft (Dex3/Prot1) | 4.30 | 2.49 |
| Balanced (Dex4/Prot3) | 3.10 | 1.46 |
| Evasive (Dex6/Prot2) | 2.75 | 1.30 |
| Armoured (Dex4/Prot5) | 2.27 | 0.87 |
| Tank (Dex4/Prot5/US5) | 1.92 | 0.67 |
| Parry (Dex5/Prot3/Parry2) | 2.05 | 0.92 |


</div>
<div>

### Defence

*Base: 12 LP*

| Attacker | Net LP/hit | ATK2K | Median |
|---|---|---|---|
| Baseline (Atk3) | 1.07 | 11.2 | 9.7 |
| Elite (Atk5, Dmg+1) | 3.43 | 3.5 | 3.3 |
| Penetrator (Atk3, Pen−3) | 1.77 | 6.8 | 5.3 |
| Drown (Atk3) | 1.56 | 7.7 | 5.8 |

</div>
</div>
