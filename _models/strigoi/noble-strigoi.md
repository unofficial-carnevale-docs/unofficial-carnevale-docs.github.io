---
layout: model
title: Noble Strigoi
permalink: /factions/strigoi/noble-strigoi/
description: "Modest fighter, but brings short-range Command denial and buffs Vampiric Attack for Vampires."
faction: strigoi
faction_title: Strigoi
nav_section: Factions
---

<p class="model-meta"><strong>Leader · Vampire</strong><span>Size 30 · 21 Ducats</span></p>

<div class="model-stats">

| AP | LP | WP | CP | Move | Dex | Atk | Prot | Mind |
|---|---|---|---|---|---|---|---|---|
| 3 | 13 | 0 | 4 | 5 | 5 | 4 | 4 | 4 |

</div>

### Weapons

| Name | Range | Damage | Pen | Special |
|---|---|---|---|---|
| Fangs | melee | — | — | — |

### Abilities

[Expert Offence(3)](/compendium/character-abilities/#expert-offence-x), [Frenzied](/compendium/character-abilities/#frenzied), [Vampiric Attack(2)](/compendium/character-abilities/#vampiric-attack-x)

**Blood Frenzy** — AURA: until end of round, all friendly chars with Vampire keyword in LOS increase their Vampiric Attack by 1, to a maximum of 2.

**Bloodline** — all friendly chars with Vampire keyword in LOS gain Companion(Noble Strigoi).

**Sanguine Sabotage** — whenever an enemy character uses a Command within 6" of this character, roll a die. On a 7+, the Command costs an extra Command Point if possible.

## Capability

EDPA **2.32** · EDPA/Duc **0.111** · ATK2K **18.4** · ATK2K/Duc **0.88**

*Damage Dealer median: EDPA **2.97** · EDPA/Duc **0.196** · ATK2K **10.9** · ATK2K/Duc **0.73***

[How these metrics are computed →](/mechanics/combat/)

<div class="model-pairs">
<div>

### EDPA by target

| Target | EDPA | Median |
|---|---|---|
| Soft (Dex3/Prot1) | 3.49 | 4.01 |
| Balanced (Dex4/Prot3) | 2.32 | 2.97 |
| Evasive (Dex6/Prot2) | 2.09 | 2.54 |
| Armoured (Dex4/Prot5) | 1.58 | 2.17 |
| Tank (Dex4/Prot5/US5) | 1.58 | 1.92 |
| Parry (Dex5/Prot3/Parry2) | 1.36 | 2.14 |

*Sustained: fangsx3*

</div>
<div>

### Defence

| Attacker | Net LP/hit | ATK2K | Median |
|---|---|---|---|
| Baseline (Atk3) | 0.71 | 18.4 | 10.9 |
| Elite (Atk5, Dmg+1) | 2.47 | 5.3 | 3.7 |
| Penetrator (Atk3, Pen−3) | 1.59 | 8.2 | 6.3 |
| Drown (Atk3) | 1.56 | 8.3 | 7.2 |

</div>
</div>
